...
import AddAdminDrawer from './components/Drawer'
...
const AdminListPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      let params = { role: 'admin', page: page + 1, limit: rowsPerPage }
      if (valueRef.current != '') {
  ...
  }, [page, rowsPerPage, valueRef.current, statusFilter, verifiedFilter])
  ...
  return (
    <>
      <Card>
        <CardHeader title='Danh sách quản trị viên' />
      ...
      <AddAdminDrawer fetchData={fetchData} open={addAdminOpen} toggle={toggleAddAdminDrawer} />
      <AdminDialog
        open={openDialog}
